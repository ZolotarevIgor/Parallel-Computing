...
double a = 0; // интервалы по х
double b = 1; //
int ni = 1000000000; //разбиение отрезка на такое количество частей (точность)
double sum = 0;
...
int main(int argc, char* argv[])
{
  ...
  if (np<2) {
    ...
    MPI_Barrier(MPI_COMM_WORLD); // горантия синхронизации и одновременного продолжения. Все ждут друг друга тут.
    t3 = MPI_Wtime();
  }
