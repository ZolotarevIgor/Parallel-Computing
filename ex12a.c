...
{
    /*
        i, j просто индексы и вспомогательные переменные
        i1, i2 диапазон вычислений для текущего процесса.
        nc = i2-i1+1 - просто диапазон вычислений
        ncm - nc-1... m - значит минус один... искользуется как граница в циклах где не нужен крайний элемент.
    */
  int i, j, i1, i2, nc, ncm, ncp, ncx;
  /*
  y0m - y_{-1} значение игрека -1 которое берем у соседнего процесса с меньшим номером.
  y0p - y_{+1} значение игрека +1 которое берем у соседнего процесса с большим номером.
  hx - шаг по х
  hx2 = hx*hx
  s1, s2 вспомогательные переменные насколько я понял
  sm, s0, sp - y(i-1), y(i), y(i+1). m - значит минус один... p - плюс один...
  */
...
// Iterations:
  ...
  do {
    ...
    if (np>1)
    //Отправляем процессу с меньшим номером y0[0] и получаем от него y0m, аналогично для для процесса с большим номером.
      BndExch1D(np,mp,1,1,1,1,&(y0[0]),&y0m,&(y0[ncm]),&y0p);
    else {
      y0m = 0.0; y0p = 0.0;
    }
    //Начинаем задавать коэффициенты a,b,c,f для текущей итерации.
    if (mp==0) {
    ...
    //Закончили задавать коэффициенты a,b,c,f для текущей итерации.


    rka = 0.0;
    //проверяем ошибку для текущего приближения по y
    //для этого подставляем текущее решение в полученные уравнения и смотрим отклонение от нуля.
    for (i=0; i<nc; i++) {
      s0 = y0[i];
    ...
    // Если ошибка меньше епсилон то выходим из итерационного процесса)
    if (rka<=eps) break;
    ...
    //Если количество итераций превысит наш максимум, то выходим.
  } while (it<=itm);
  //stop iterations
