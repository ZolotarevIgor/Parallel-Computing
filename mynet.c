...
/*
Функция разбиения отрезка.
Дан диапазон для итераций от ia до ib (шаг 1). Возвращается диапазон который должен вычислить
текущий процесс ([i1, i2]), при условии что всего np процессов и текущий процесс имеет номер mp.
nc - это количество итераций для нашего процесса
*/
